...
## 에셋 번들 매니저에 도전해보자! part2. 코드편 ##

에셋 번들 매니저 드디어 코드입니다.

### 에셋 번들 매니저 코드 포스팅 순서 ###

AssetBundleManager.cs 에다 코드를 때려받는 미친 짓을 하고 말았습니다. ㅠㅠ

덕분에 에셋 번들 매니저는 200줄이 넘는 비효율적인 코드로 작성되었고, 결국 뒤집는 작업을 했습니다.

UnityManual 에도 실려 있는 에셋번들 매니저의 기초가 되는 부분만을 에셋 번들 매니저에 남겼습니다.

이 기초가 되는 부분은 에셋 번들의 로딩까지입니다.

이후에 에셋번들이 적재되고 일정기간 사용되지 않으면 자동으로 날려주는 에셋번들 타이머.

에셋 번들에서 별도의 양식 없이 에셋을 비동기 로드해주는 에셋 로더를 만들었습니다.

이 두 매니저는 스타크래프트의 애드온 (커맨드 센터의 컴셋스테이션)같은 Add-On으로서 

에셋번들 매니저와 서로 물린 채로 상호작용하도록 만들었습니다. 이렇게 코드를 한번 뒤집고 분산시켜 정리하니 훨씬 짧고 가독성도 오르더군요.

## 에셋 번들 매니저에 도전해보자! part3-1. 확장편(타임 컨트롤) ##

처음에 에셋 번들을 불러오는 과정에서 그 에셋번들의 생명주기를 설정하면, 알아서 생명주기가 끝날 때에 에셋 번들을 Unload() 해주는 기능을 구현하였습니다.

## 에셋 번들 매니저에 도전해보자! part3-2. 확장편(에셋 로더) ##

마지막입니다.

결론

이 매니저 쓰실 분들은 AssetBundeManager, AssetLoadManager은 필수로 사용하시고, 정 에셋번들 관리가 어렵다고 하시는 분들은 AssetBundleTimeManager 까지 추가하셔서 사용하시는 것이 좋아보입니다.